...
def main():
    ...
    args.output_dir = os.path.join(args.output_dir, args.dataset_name, args.model_name_or_path.split('/')[-1],
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if args.with_tracking:
                ...
                if completed_steps % 5 == 0 and completed_steps < 10000:
                    loss_file.write(f"{completed_steps}, {loss.detach().float()}\n")
            loss.backward()
            optimizer.step()
            lr_scheduler.step()
            optimizer.zero_grad()
            ...
            if isinstance(checkpointing_steps, int):
                if completed_steps % checkpointing_steps == 0:
                    ...
                    if args.output_dir is not None:
                        output_dir = os.path.join(args.output_dir, output_dir, 'model.bin')
        ...
        for step, batch in enumerate(eval_dataloader):
            with torch.no_grad():
                ...
                if not args.pad_to_max_length:
                    ...
                    labels = pad_across_processes(batch["labels"], dim=1, pad_index=tokenizer.pad_token_id)
        ...
        print(result)
        metric_file.write(f'{epoch}: {result}')
